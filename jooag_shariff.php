...
class plgSystemJooag_Shariff extends JPlugin
{
	...
	public function onContentPrepare($context, &$article, &$params, $page = 0)
	{
        if(preg_match_all('/{shariff\ ([^}]+)\}|\{shariff\}/', $article->text, $matches)
            and $this->getAccessGeneral('com_shorttag', $article, 'top') == 1)
		{
	...
	private function getAccessGeneral($context, $article, $position)
	{
	    ...
	    if(in_array($position, $this->params->get('output_position')))
		{
		...
		if($this->params->get('com_shorttag') == 1 and $context == 'com_shorttag')
		{
			return 1;
		}
	...
	public function onExtensionBeforeSave($context, $table, $isNew)
	{
		if($table->name == 'PLG_JOOAG_SHARIFF')
		{
			...
			$json->services = json_decode(json_encode(array_values($json->services)));
